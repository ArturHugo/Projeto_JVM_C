  ...
  for(cp = constant_pool + 1; cp < constant_pool + cp_count; cp++) {
    ...
    switch(cp->tag) {
      ...
      case CONSTANT_UTF8:
        ...
        cp->utf8_info.bytes =
            (u1*) malloc((cp->utf8_info.length + 1) * sizeof(u1));
...
u1** getUtf8Strings(ConstantPoolInfo* constant_pool, uint16_t index) {
  ...
  switch(constant_pool[index].tag) {
    case CONSTANT_CLASS:
      ...
      utf8_strings[0] =
          constant_pool[class_name_index].utf8_info.bytes;
      break;
    ...
    case CONSTANT_NAME_AND_TYPE:
      ...
      utf8_strings[1] =
          constant_pool[descriptor_index].utf8_info.bytes;
      break;
  ...
  if((constant_pool[index].tag == CONSTANT_FIELDREF) ||
    ...
    u2 class_name_index =
        constant_pool[class_index].class_info.name_index;
    utf8_strings[0] = constant_pool[class_name_index].utf8_info.bytes;

    u2 field_name_index = constant_pool[name_and_type_index]
                              .name_and_type_info.name_index;
    utf8_strings[1] = constant_pool[field_name_index].utf8_info.bytes;

    u2 descriptor_index = constant_pool[name_and_type_index]
...
void printConstantPoolInfo(ConstantPoolInfo* constant_pool,
                           int               index) {
  ...
  switch(constant_pool[index].tag) {
    ...
    case CONSTANT_DOUBLE:
      ...
      double_bytes =
          ((int64_t) double_high << 32) | ((int64_t) double_low);
