...
void set_fields(FILE *fp, ClassFile *classFile, ConstantPoolInfo* cp) {
...
void set_methods(FILE *fp, ClassFile *classFile, ConstantPoolInfo* cp) {
    ...
    for (u2 i = 0; i < classFile->methods_count; i++) {
        ...
        //PERGUNTAR PROS MLK
        //classFile->methods[i].attributes = readAttributes(classFile->methods[i].attributes_count, fd , cp);
        }
    }
}
...
ClassFile* readClassFile(FILE * fp){
  ...
  ConstantPoolInfo* cp = readConstantPool(class_file-> constant_pool_count, fp);
