...
void test_read_inner_classes_attributes(void) {
  fd->seek                     = 0x2a4;
  AttributeInfo* attributes    = readAttributes(1, fd, class_file->constant_pool);
  ...
  unsigned char* inner_class_name =
      class_file->constant_pool[inner_class_name_index].utf8_info.bytes;

  TEST_ASSERT_EQUAL(1, number_of_classes);
  TEST_ASSERT_EQUAL_STRING("Attributes$InnerClass", inner_class_name);
